...
export async function createNatsConnection(): Promise<NatsConnection> {
  try {
    ...
    console.error(
      "Erreur lors de l'ouverture de la connexion NATS :",
      error.message
    );
  }
}
...
export async function closeNatsConnection(): Promise<void> {
  try {
    ...
    console.error(
      "Erreur lors de la fermeture de la connexion NATS :",
      error.message
    );
  }
}
...
export async function publishMessage(
  ...
  try {
    ...
    console.log(
      "Erreur lors de la publication (publish) du message NATS :",
      error.message
    );
    throw new Error("Erreur lors de la publication du message NATS");
  }
}
...
export async function requestMessage(subject: string, data: any): Promise<any> {
  try {
    ...
    console.error(
      "Erreur lors de la publication (request) du message NATS :",
      error.message
    );
  }
}
